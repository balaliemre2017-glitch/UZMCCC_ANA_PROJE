...
def get_user():
    try:
        ...
        cur = con.cursor()
        cur.execute("SELECT auth_type, email, phone, aktif FROM users WHERE platform=?", ("github",))
        row = cur.fetchone()
        con.close()
        return row
    except:
...
class Worker:
    def __init__(self):
        ...
        self.yetenekler = {"commit": true, "push": true, "yedek": true}
    ...
    def paylas(self, path, aciklama):
        ...
        try:
            ...
            aktif = 1
        if not aktif:
            print(f"[GITHUB] Kapalı")
            return False
        kimlik = email if auth_type=='email' else phone
        print(f"[GITHUB] PAYLASILDI: {path} -> {kimlik} | {aciklama[:70]}")
        # Gerçek API entegrasyonu buraya
        try:
            p = pathlib.Path(path)
            if p.is_dir():
                (p / f"github_paylasildi.txt").write_text(f"OK {aciklama}", encoding="utf-8")
        except:
            pass
        return True
